...
public class Bid {
  ...
  public void save() {
		Bid.connection.openConnection();
		if(/*Bid.fetch("bid_code = " + this.bid_code).size() == 0*/ true) {
			Bid.connection.executeUpdate("INSERT INTO Bid VALUES (" + /*this.bid_code*/ 2 + ", " + /*this.sale_code*/ 2 + ", " + /*this.user.getCode()*/ 2 + ", " + /*this.accepted*/ 1 + ", '" + this.amount + "', TO_DATE('" + this.tstamp.toSqlDateTime() + "', 'YYYY-MM-DD HH24:MI')" + ")");
		} else {
			Bid.connection.executeUpdate("UPDATE Bid SET bid_code = " + /*this.bid_code*/ 1 + ", sale_code = " + /*this.sale_code*/ 1 + ", user_code" + this.user.getCode() + ", accepted = " + this.accepted + ", amount = " + this.amount + ", tstamp = TO_DATE('" + this.tstamp.toSqlDateTime() + "', 'YYYY-MM-DD HH24:MI')" + " WHERE bid_code = " + this.bid_code + "");
		}
		Bid.connection.closeConnection();
	}
